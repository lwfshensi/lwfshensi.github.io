...
module LogStash module PluginMixins module Jdbc
  ...
  class NormalStatementHandler < StatementHandler
    ...
    def perform_query(db, sql_last_value, jdbc_paging_enabled, jdbc_page_size,subquery_paging_enabled,sum_statement,logger)
      query = build_query(db, sql_last_value)
      if subquery_paging_enabled
        data_sum = db[sum_statement].get(:sum)
        logger.info("data_sum is,:#{data_sum}")

        data_offset = 0
        while data_offset < data_sum do
          parameters[:data_offset] = data_offset
          parameters[:jdbc_page_size] = jdbc_page_size
          parameters[:jdbc_page_size] = jdbc_page_size
          sub_page_query = db[statement, parameters]
